...
public class GetIntervento extends AsyncTask<Long, Void, Intervento> {

    private Context context;

    public GetIntervento(Context context) {
	...
	String[] projection = new String[] { InterventoDB.Fields._ID,
		InterventoDB.Fields.CANCELLATO, InterventoDB.Fields.CHIUSO,
		...
		InterventoDB.Fields.MODALITA, InterventoDB.Fields.MOTIVO,
		InterventoDB.Fields.NOMINATIVO, InterventoDB.Fields.NOTE,
		InterventoDB.Fields.ORA_FINE, InterventoDB.Fields.ORA_INIZIO,
		InterventoDB.Fields.ORE, InterventoDB.Fields.PRODOTTO,
	...
	if (cursor.moveToFirst()) {
	    intervento
		    .setmCancellato(cursor.getInt(cursor
			    .getColumnIndex(InterventoDB.Fields.CANCELLATO)) == 1 ? true
			    : false);
	    intervento.setmChiuso(cursor.getInt(cursor
		    .getColumnIndex(InterventoDB.Fields.CHIUSO)) == 1 ? true
		    : false);
	    intervento
		    .setmCostoAccessori(BigDecimal.valueOf(cursor.getDouble(cursor
			    .getColumnIndex(InterventoDB.Fields.COSTO_ACCESSORI))));
	    intervento
		    .setmCostoComponenti(BigDecimal.valueOf(cursor.getDouble(cursor
			    .getColumnIndex(InterventoDB.Fields.COSTO_COMPONENTI))));
	    intervento
		    .setmCostoManodopera(BigDecimal.valueOf(cursor.getDouble(cursor
			    .getColumnIndex(InterventoDB.Fields.COSTO_MANODOPERA))));
	    intervento.setmDataOra(cursor.getLong(cursor
		    .getColumnIndex(InterventoDB.Fields.DATA_ORA)));
	    intervento.setmFine(cursor.getLong(cursor
		    .getColumnIndex(InterventoDB.Fields.ORA_FINE)));
	    intervento.setmFirma(cursor.getBlob(cursor
		    .getColumnIndex(InterventoDB.Fields.FIRMA)));
	    intervento.setmIdCliente(cursor.getLong(cursor
		    .getColumnIndex(InterventoDB.Fields.CLIENTE)));
	    intervento.setmIdIntervento(params[0]);
	    intervento.setmIdTecnico(cursor.getLong(cursor
		    .getColumnIndex(InterventoDB.Fields.TECNICO)));
	    intervento.setmImporto(BigDecimal.valueOf(cursor.getDouble(cursor
		    .getColumnIndex(InterventoDB.Fields.IMPORTO))));
	    intervento.setmInizio(cursor.getLong(cursor
		    .getColumnIndex(InterventoDB.Fields.ORA_INIZIO)));
	    intervento.setmIva(BigDecimal.valueOf(cursor.getDouble(cursor
		    .getColumnIndex(InterventoDB.Fields.IVA))));
	    intervento.setmModalita(cursor.getString(cursor
		    .getColumnIndex(InterventoDB.Fields.MODALITA)));
	    intervento.setmMotivo(cursor.getString(cursor
		    .getColumnIndex(InterventoDB.Fields.MOTIVO)));
	    intervento.setmNominativo(cursor.getString(cursor
		    .getColumnIndex(InterventoDB.Fields.NOMINATIVO)));
	    intervento.setmNote(cursor.getString(cursor
		    .getColumnIndex(InterventoDB.Fields.NOTE)));
	    intervento.setmOre(BigDecimal.valueOf(cursor.getDouble(cursor
		    .getColumnIndex(InterventoDB.Fields.ORE))));
	    intervento.setmProdotto(cursor.getString(cursor
		    .getColumnIndex(InterventoDB.Fields.PRODOTTO)));
	    intervento.setmRifFattura(cursor.getString(cursor
		    .getColumnIndex(InterventoDB.Fields.RIFERIMENTO_FATTURA)));
	    intervento
		    .setmRifScontrino(cursor.getString(cursor
			    .getColumnIndex(InterventoDB.Fields.RIFERIMENTO_SCONTRINO)));
	    intervento.setmSaldato(cursor.getInt(cursor
		    .getColumnIndex(InterventoDB.Fields.SALDATO)) == 1 ? true
		    : false);
	    intervento.setmTipologia(cursor.getString(cursor
		    .getColumnIndex(InterventoDB.Fields.TIPOLOGIA)));
	    intervento.setmTotale(BigDecimal.valueOf(cursor.getDouble(cursor
