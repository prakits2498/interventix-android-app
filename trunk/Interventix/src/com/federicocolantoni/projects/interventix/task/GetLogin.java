		...
		if (!defaultPrefs.getBoolean(prefs_auto_login, false)) {
			...
			try {
				...
				if (response.get("response").toString().equalsIgnoreCase("success")) {
					...
					String[] selectionArgs = new String[] { UtenteDB.UTENTE_ITEM_TYPE, mUsername };
					...
					if (cursor.getCount() > 0) {
						...
						String[] selectionUpdateArgs = new String[] { "" + data.get("idutente") };
						...
						localPrefs.edit().putLong(Constants.USER_ID, data.getLong("idutente")).putString(Constants.USERNAME, mUsername).putString(Constants.PASSWORD, mPassword)
								.commit();
