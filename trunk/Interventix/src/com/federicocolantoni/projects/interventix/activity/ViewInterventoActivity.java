		...
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			...
			if (manager.getBackStackEntryCount() == 1) {
				...
				if (!interv_modified && !dett_interv_modified) {
					...
					writeDB.startDelete(Constants.TOKEN_RIPRISTINO_INTERVENTO, null, RipristinoInterventoDB.CONTENT_URI, RipristinoInterventoDB.Field.TYPE + "=?",
							new String[] { RipristinoInterventoDB.RIPRISTINO_INTERVENTO_ITEM_TYPE });
				...
				try {
					cursorDB = new ManagedAsyncTask<String, Void, Cursor>((ActionBarActivity) getActivity()) {
						...
						protected Cursor doInBackground(String... params) {
							...
							cursor = cr.query(RipristinoInterventoDB.CONTENT_URI,
									new String[] { RipristinoInterventoDB.Field._ID, RipristinoInterventoDB.Field.BACKUP_INTERVENTO }, RipristinoInterventoDB.Field.TYPE + "=?",
				...
				if (cursorDB != null) {
					...
					if (first) {
						try {
							...
							queryHandlerDB.startUpdate(Constants.TOKEN_RIPRISTINO_INTERVENTO, null, InterventoDB.CONTENT_URI, valuesIntervento, InterventoDB.Fields.TYPE
									+ "=? AND " + InterventoDB.Fields.ID_INTERVENTO + "=?",
									new String[] { InterventoDB.INTERVENTO_ITEM_TYPE, "" + intervRipristino.getLong(InterventoDB.Fields.ID_INTERVENTO.toString()) });
							...
							for (int i = 0; i < dettagliIntervento.length(); i++) {
								...
								queryHandlerDB.startUpdate(
										Constants.TOKEN_RIPRISTINO_INTERVENTO,
										null,
										DettaglioInterventoDB.CONTENT_URI,
										valuesDettaglio,
										DettaglioInterventoDB.Fields.TYPE + "=? AND " + DettaglioInterventoDB.Fields.ID_DETTAGLIO_INTERVENTO + "=?",
										new String[] { DettaglioInterventoDB.DETTAGLIO_INTERVENTO_ITEM_TYPE,
												"" + dettaglio.getLong(DettaglioInterventoDB.Fields.ID_DETTAGLIO_INTERVENTO.toString()) });
							}

							queryHandlerDB.startDelete(Constants.TOKEN_RIPRISTINO_INTERVENTO, null, RipristinoInterventoDB.CONTENT_URI, RipristinoInterventoDB.Field.TYPE + "=?",
									new String[] { RipristinoInterventoDB.RIPRISTINO_INTERVENTO_ITEM_TYPE });

							queryHandlerDB.startDelete(Constants.TOKEN_RIPRISTINO_INTERVENTO, null, DettaglioInterventoDB.CONTENT_URI,
									DettaglioInterventoDB.Fields.ID_DETTAGLIO_INTERVENTO + "<?", new String[] { "0" });
						} catch (JSONException e) {
