...
public class MainActivity extends BaseActivity implements OnLoginListener {
	...
	if (username.getText().toString().length() == 0 || password.getText().toString().length() == 0)
		...
		switch (info.getType()) {
			...
			if (info.isAvailable() && info.isConnected())
				...
				new GetLogin(MainActivity.this).execute(json_req, username.getText().toString(), password.getText().toString());
			    }
			    catch (Exception e) {
				e.printStackTrace();
				BugSenseHandler.sendException(e);
				...
				if (usrnm.equals(username.getText().toString()) && psswrd.equals(password.getText().toString())) {
				    ...
				    InterventixToast.makeToast(this, "Accesso in modalità offline", Toast.LENGTH_LONG);
	...
	protected Integer doInBackground(String... strings) {
		...
		try {
		    ...
		    JSONObject jsonResponse = Utils.connectionForURL(strings[0], url);
		    
		    if (jsonResponse.get("response").toString().equalsIgnoreCase("success")) {
			...
			if (cursor.getCount() > 0) {
			    ...
			    values.put(UtenteDB.Fields.NOME, (String) data.get("nome"));
			    values.put(UtenteDB.Fields.COGNOME, (String) data.get("cognome"));
			    values.put(UtenteDB.Fields.USERNAME, (String) data.get("username"));
			    values.put(UtenteDB.Fields.CANCELLATO, (Boolean) data.get("cancellato"));
			    values.put(UtenteDB.Fields.REVISIONE, (Long) data.get("revisione"));
			    values.put(UtenteDB.Fields.EMAIL, (String) data.get("email"));
			    values.put(UtenteDB.Fields.TIPO, (String) data.get("tipo"));
			    values.put(UtenteDB.Fields.CESTINATO, (Boolean) data.get("cestinato"));
			    ...
			    values = new ContentValues();
			    values.put(UtenteDB.Fields.ID_UTENTE, (Long) data.get("idutente"));
