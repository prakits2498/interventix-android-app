		...
		try {
			...
			if (idIntervento != 0L) {
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
					...
					InterventoController.controller.setCliente(new GetClienteAsyncTask(this).executeOnExecutor(AsyncTask.SERIAL_EXECUTOR,
							InterventoController.controller.getIntervento().getIdCliente()).get());
					InterventoController.controller.setTecnico(new GetUtenteAsyncTack(this).executeOnExecutor(AsyncTask.SERIAL_EXECUTOR,
							InterventoController.controller.getIntervento().getIdTecnico()).get());
					InterventoController.controller.setListaDettagli(new GetListaDettagliInterventoAsyncTask(this)
							.executeOnExecutor(AsyncTask.SERIAL_EXECUTOR, InterventoController.controller.getIntervento().getIdIntervento()).get().getListDetails());
				} else {
					...
					InterventoController.controller.setListaDettagli(new GetListaDettagliInterventoAsyncTask(this)
							.execute(InterventoController.controller.getIntervento().getIdIntervento()).get().getListDetails());
				}
	...
	private void getMaxIdIntervento() {
		...
		new AsyncTask<Void, Void, Long>() {
			...
			protected Long doInBackground(Void... params) {
				...
				String[] projection = new String[] { InterventoDB.Fields.ID_INTERVENTO };

				String selection = Fields.TYPE + "=?";

				String[] selectionArgs = new String[] { InterventoDB.INTERVENTO_ITEM_TYPE };
	...
	private void getMaxNumeroIntervento() {
		...
		new AsyncTask<Void, Void, Long>() {
			...
			protected Long doInBackground(Void... params) {
				...
				String[] projection = new String[] { InterventoDB.Fields.NUMERO_INTERVENTO };
