...
public class GetCostsIntervento extends AsyncTask<Long, Void, Intervento> {
	...
	protected Intervento doInBackground(Long... params) {
		...
		if (cursor.moveToFirst()) {
			...
			costsIntervento.setmImporto(BigDecimal.valueOf(cursor
					.getDouble(cursor
							.getColumnIndex(InterventoDB.Fields.IMPORTO))));
			costsIntervento.setmIva(BigDecimal.valueOf(cursor.getDouble(cursor
					.getColumnIndex(InterventoDB.Fields.IVA))));
			costsIntervento.setmTotale(BigDecimal.valueOf(cursor
					.getDouble(cursor
