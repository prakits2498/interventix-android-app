		...
		new ManagedAsyncTask<Long, Void, Integer>(HomeActivity.this) {
			...
			private void addInterventions(JSONObject responseIntervs, RuntimeExceptionDao<Intervento, Long> interventoDao) {
				...
				try {
					...
					RuntimeExceptionDao<DettaglioIntervento, Long> dettaglioInterventoDao = com.federicocolantoni.projects.interventix.Interventix_.getDbHelper()
			...
			protected void onPostExecute(Integer result) {
				...
				if (result != RESULT_OK) {
					...
					qb.selectColumns(new String[] { "numero", "dataora", "cliente", "conflitto", "modificato", "nuovo" });
